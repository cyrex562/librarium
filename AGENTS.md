...
- `crates/codex-server`: main Actix Web backend, default workspace member
- `crates/codex-types`: shared Rust DTOs and parser traits
- `crates/codex-client`: HTTP and WebSocket client crate
- `crates/codex-tauri`: desktop shell that embeds the frontend and server
...
- Treat `crates/codex-server/src/services` as business logic, `routes` as thin transport adapters, and `models` / `codex-types` as shared contracts.
...
- Backend tests: `cargo test -p codex-server`
...
- The server reads `config.toml` by default, or `CODEX_CONFIG` / `--config`.
- Auth, JWT, LDAP, OIDC, CORS, vault paths, and TLS are configured in `crates/codex-server/src/config/mod.rs`.
...
- Auth and session behavior: `crates/codex-server/src/routes/auth.rs`, `middleware/auth.rs`, `routes/totp.rs`
- Filesystem mutation paths: `crates/codex-server/src/services/file_service.rs`
- Search index consistency: `crates/codex-server/src/services/search_service.rs`
- Reindex and entity sync: `crates/codex-server/src/services/reindex_service.rs`
