...
fn extract_entity_meta(content: &str) -> EntityMeta {
    ...
    let entity_type = yaml_block.lines().find_map(|line| {
        ...
        if let Some(rest) = line
            .strip_prefix("librarium_type:")
            .or_else(|| line.strip_prefix("codex_type:"))
        {
    ...
    for line in yaml_block.lines() {
        ...
        if trimmed.starts_with("librarium_labels:") || trimmed.starts_with("codex_labels:") {
            in_labels = true;
            continue;
        }
        if in_labels {
...
mod tests {
    ...
    #[test]
    fn test_extract_entity_meta_no_librarium_type() {
        let content = "---\ntitle: My Note\nauthor: Alice\n---\n# Content";
        let meta = extract_entity_meta(content);
        assert!(meta.entity_type.is_none());
        assert!(meta.labels.is_empty());
    ...
    #[test]
    fn test_extract_entity_meta_with_librarium_type() {
