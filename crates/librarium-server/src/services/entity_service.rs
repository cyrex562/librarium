...
impl EntityService {
    ...
    /// keys `librarium_type`, `librarium_labels`, and `librarium_plugin` are extracted;
    ...
    pub async fn upsert(
        ...
        let entity_type = match frontmatter_str(obj, "librarium_type", "codex_type") {
            Some(t) => t.to_string(),
            None => return Ok(None), // Not a typed entity
        };

        let plugin_id = frontmatter_str(obj, "librarium_plugin", "codex_plugin")
            .unwrap_or("unknown")
            .to_string();

        let labels: Vec<String> = frontmatter_array(obj, "librarium_labels", "codex_labels")
            .map(|arr| {
                arr.iter()
...
mod tests {
    ...
    fn test_parse_frontmatter_valid() {
        ...
        assert_eq!(fm["librarium_type"].as_str(), Some("character"));
    ...
    fn test_parse_frontmatter_leading_whitespace() {
        ...
        assert_eq!(fm["librarium_type"].as_str(), Some("location"));
    }
