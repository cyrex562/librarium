...
impl ReindexService {
    ...
    pub async fn index_file(
        ...
        // File has no Librarium entity marker — remove entity if it existed before.
...
async fn collect_md_files(root: &str) -> Vec<String> {
    let excluded_dirs = [".git", ".obsidian", ".trash", "node_modules", ".librarium"];
...
mod tests {
    ...
    async fn test_reindex_vault_skips_non_entity_files() {
        ...
        // Normal markdown without librarium_type
        std::fs::write(
        ...
        assert!(
            entities.is_empty(),
            "plain md with no librarium_type should not be indexed"
        );
    }
    ...
    #[tokio::test]
    async fn test_index_file_removes_entity_when_librarium_type_removed() {
        let temp = TempDir::new().unwrap();
        let db = setup_db(&temp).await;
        let vault_dir = temp.path().join("vault");
        std::fs::create_dir_all(&vault_dir).unwrap();
        ...
        // Overwrite without librarium_type
        ...
        assert!(
            entity.is_none(),
            "entity should be removed when librarium_type is absent"
        );
    }
