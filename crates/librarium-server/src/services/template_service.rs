...
fn generate_minimal_template(schema: &crate::models::schema::EntityTypeSchema) -> String {
    ...
    writeln!(fm, "---").unwrap();
    writeln!(fm, "librarium_type: {}", schema.name).unwrap();
    writeln!(fm, "librarium_plugin: {}", schema.plugin_id).unwrap();
    if !schema.labels.is_empty() {
        writeln!(fm, "librarium_labels:").unwrap();
    ...
    writeln!(fm, "---").unwrap();
    writeln!(fm).unwrap();
    writeln!(fm, "<!-- librarium:prose:begin -->").unwrap();
    writeln!(fm).unwrap();
    writeln!(fm, "<!-- librarium:prose:end -->").unwrap();
    fm
}
...
mod tests {
    ...
    #[test]
    fn test_minimal_template_contains_librarium_type() {
        let schema = make_schema("character", "worldbuilding", vec![], vec![]);
        let tmpl = generate_minimal_template(&schema);
        assert!(
            tmpl.contains("librarium_type: character"),
            "should set librarium_type"
        );
    }

    #[test]
    fn test_minimal_template_contains_plugin_id() {
        let schema = make_schema("location", "com.librarium.worldbuilding", vec![], vec![]);
        let tmpl = generate_minimal_template(&schema);
        assert!(tmpl.contains("librarium_plugin: com.librarium.worldbuilding"));
    }
    ...
    fn test_minimal_template_labels_emitted_when_present() {
        ...
        let tmpl = generate_minimal_template(&schema);
        assert!(
            tmpl.contains("librarium_labels:"),
            "should emit librarium_labels section"
        );
    ...
    fn test_minimal_template_contains_prose_markers() {
        ...
        let tmpl = generate_minimal_template(&schema);
        assert!(tmpl.contains("<!-- librarium:prose:begin -->"));
        assert!(tmpl.contains("<!-- librarium:prose:end -->"));
    }
    ...
    async fn test_get_template_returns_fallback_when_no_file() {
        ...
        assert!(
            content.contains("librarium_type:"),
    ...
    async fn test_get_template_reads_real_file() {
        ...
        let schema = EntityTypeSchema {
            id: "character".into(),
            plugin_id: "com.librarium.worldbuilding".into(),
            name: "character".into(),
            icon: None,
            color: None,
