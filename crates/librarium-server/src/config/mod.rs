...
impl AppConfig {
    /// Load defaults plus LIBRARIUM__* environment overrides.
    ///
    ...
    pub fn load_from_env_or_default() -> anyhow::Result<Self> {
        let cfg = config::Config::builder()
            ...
            .add_source(config::Environment::with_prefix("LIBRARIUM").separator("__"))
            .build()
    ...
    /// Environment variable overrides (`LIBRARIUM__*`) are applied on top.
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.database.path, "./librarium.db");
    ...
    fn make_paths(temp: &TempDir) -> LibrariumPaths {
        LibrariumPaths {
            config_dir: temp.path().join("config"),
            data_dir: temp.path().join("data"),
            cache_dir: temp.path().join("cache"),
            default_vault_dir: temp.path().join("Documents/Librarium"),
        }
    }
    ...
    fn test_load_from_dirs_no_config_file_returns_defaults() {
        ...
        assert!(config.database.path.contains("librarium.db"));
        assert!(
            config.vault.base_dir.contains("Librarium"),
    ...
    fn test_default_for_dirs_paths_resolve_relative_to_codex_paths() {
        ...
        assert!(
            config.database.path.contains("librarium.db"),
            "database path should include librarium.db"
        );
        assert!(
