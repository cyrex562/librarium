...
- [x] LIB-105: display the folder choosing window for moving folder list as a tree rather than flat list — MoveToFolderModal now renders an expand/collapse tree (chevrons, vault root open by default, `toggleExpand`) instead of a fully-flattened indented list; an active filter still shows a flat list of matches with full paths for context.
- [x] LIB-106: when opening the folder choosing window, automatically put cursor in filter box so that typing begins typing a filter pattern — added `autofocus` to the MoveToFolderModal filter field (matches the QuickSwitcher/SearchModal pattern; v-dialog mounts content lazily so it fires on each open).
- [x] LIB-107: when in the folder choosing window, enable arrow key navigation and tab navigation between elements, such that after typing a fitler, the user can arrow down or tab to the list. Once a folder is selected, typing enter should confirm it and then close the window. — MoveToFolderModal now handles keyboard nav (card-level `@keydown`): Arrow Up/Down move the highlight (skipping disabled rows, clamped), Right/Left expand/collapse or jump to parent, Enter confirms the move and closes. The list is a tab stop (`tabindex=0`, focuses first row); changing the filter resets the highlight so ArrowDown lands on the first match; the highlighted row scrolls into view.
