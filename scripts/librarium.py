#!/usr/bin/env python3
"""
librarium — unified CLI for building, deploying, configuring, and testing Librarium.

Usage:
    python scripts/librarium.py [COMMAND] [OPTIONS]
    ./scripts/librarium.py build --server --release
    ./scripts/librarium.py deploy librarium-test-a --build-first
    ./scripts/librarium.py test --all
    ./scripts/librarium.py configure --target librarium-test-a --show
    ./scripts/librarium.py doctor --target librarium-test-a
"""
...
SYSTEMD_TEMPLATE = REPO_ROOT / "deploy" / "systemd" / "librarium.service.template"
...
SERVICE_NAME = "librarium"
BIN_NAME = "librarium"
DESKTOP_BIN_NAME = "librarium-desktop"
...
class LibrariumError(RuntimeError):
    pass
...
class RemoteDataSnapshot:
    ...
    legacy_app_db_present: bool
    legacy_app_vault_file_count: int
    legacy_codex_db_present: bool
    legacy_codex_vault_file_count: int
    legacy_codex_backup_count: int
...
def run_cmd(
    ...
    *,
    ...
    if check and result.returncode != 0:
        ...
        raise LibrariumError(detail or f"Command exited {result.returncode}: {' '.join(command)}")
    return result
...
def ensure_tool(name: str) -> None:
    if shutil.which(name) is None:
        raise LibrariumError(f"Required tool '{name}' not found on PATH")
...
def load_targets(targets_file: Path = TARGETS_FILE) -> dict[str, DeploymentTarget]:
    if not targets_file.exists():
        raise LibrariumError(f"Targets file not found: {targets_file}")
...
def pick_target(
    ...
    if name:
        if name not in targets:
            raise LibrariumError(f"Unknown target '{name}'. Run `librarium.py targets` to list.")
        return targets[name]
...
def build_frontend() -> None:
    ...
    if not (FRONTEND_BUILD_OUTPUT / "index.html").exists():
        raise LibrariumError(f"Frontend build output missing: {FRONTEND_BUILD_OUTPUT}")
...
def build_server(
    *,
    ...
    run_cmd(["cargo", "clean", "-p", "librarium-server"], cwd=REPO_ROOT)
    ...
    if effective_target and not (is_native_linux and effective_target == LINUX_TARGET):
        ...
        _ensure_docker()
        run_cmd(
            ["cross", "build", "--target", effective_target, *profile_flag, "-p", "librarium-server"],
            cwd=REPO_ROOT,
        )
        binary = REPO_ROOT / "target" / effective_target / profile_dir / BIN_NAME
    else:
        run_cmd(
            ["cargo", "build", *profile_flag, "-p", "librarium-server"],
            cwd=REPO_ROOT,
        )
        binary = REPO_ROOT / "target" / profile_dir / BIN_NAME

    if not binary.exists():
        raise LibrariumError(f"Server binary not found: {binary}")
...
def build_server_remote(target: DeploymentTarget) -> Path:
    ...
    remote_root = f"{target.tmp_dir}/librarium-build-{build_id}"
    remote_archive = f"{remote_root}/source.tar.gz"
    remote_src = f"{remote_root}/src"
    remote_bin = f"{remote_root}/librarium"
    ...
    try:
        ...
        ssh_cmd(
            ...
            textwrap.dedent(f"""
                ...
                cp {shell_quote(remote_src + "/target/release/librarium")} {shell_quote(remote_bin)}
            """).strip(),
        )
    ...
    if not local_bin.exists():
        raise LibrariumError(f"Remote-built binary not found locally: {local_bin}")
...
def build_desktop(*, release: bool = True, cross_target: str | None = None) -> Path:
    ...
    if effective_target and not (is_native_linux and effective_target == LINUX_TARGET):
        ...
        _ensure_docker()
        run_cmd(
            ["cross", "build", "--target", effective_target, *profile_flag, "-p", "librarium-desktop"],
            cwd=REPO_ROOT,
        )
        binary = REPO_ROOT / "target" / effective_target / profile_dir / DESKTOP_BIN_NAME
    else:
        run_cmd(
            ["cargo", "build", *profile_flag, "-p", "librarium-desktop"],
            cwd=REPO_ROOT,
        )
        binary = REPO_ROOT / "target" / profile_dir / DESKTOP_BIN_NAME

    if not binary.exists():
        raise LibrariumError(f"Desktop binary not found: {binary}")
    ...
    *,
    ...
    if remote_build:
        ...
        if not skip_support:
            ...
            run_cmd(["cargo", "build", "-p", "librarium-client"], cwd=REPO_ROOT)
            run_cmd(["cargo", "build", "-p", "librarium-types"], cwd=REPO_ROOT)
    ...
    deploy_dir.mkdir()
    tarball = deploy_dir / f"librarium-{release_id}.tar.gz"
...
def load_existing_artifacts(target: DeploymentTarget) -> BuildArtifacts:
    ...
    if not manifest_path.exists():
        raise LibrariumError("dist/manifest.json missing — run `librarium.py build` first.")
    m = json.loads(manifest_path.read_text(encoding="utf-8"))
    bin_copy = DIST_DIR / m["server"]["artifact"]
    tarball = DIST_DIR / "deploy" / f"librarium-{m['release_id']}.tar.gz"
    for p in [bin_copy, tarball]:
        if not p.exists():
            raise LibrariumError(f"Expected dist artifact missing: {p}")
    return BuildArtifacts(
        target_name=target.name,
        server_binary=bin_copy,
...
def create_remote_backup(
    ...
    *,
    ...
    backup_name = f"librarium-backup-{timestamp}-{safe_label}.tar.gz"
    ...
    backup_script = textwrap.dedent(f"""
        ...
        if [ {1 if stop_services else 0} -eq 1 ]; then
            if [ -f docker-compose.yml ]; then
                if docker info >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
                    docker compose stop librarium >/dev/null 2>&1 && stopped_docker=1 || true
                elif docker info >/dev/null 2>&1 && command -v docker-compose >/dev/null 2>&1; then
                    docker-compose stop librarium >/dev/null 2>&1 && stopped_docker=1 || true
                elif sudo -n docker info >/dev/null 2>&1 && sudo -n docker compose version >/dev/null 2>&1; then
                    sudo docker compose stop librarium >/dev/null 2>&1 && stopped_docker=1 || true
                elif command -v docker-compose >/dev/null 2>&1 && sudo -n docker info >/dev/null 2>&1 && sudo -n docker-compose version >/dev/null 2>&1; then
                    sudo docker-compose stop librarium >/dev/null 2>&1 && stopped_docker=1 || true
                fi
            fi
        ...
        if [ {1 if restart_after else 0} -eq 1 ]; then
            ...
            if [ "$stopped_docker" -eq 1 ]; then
                if docker info >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then docker compose up -d librarium >/dev/null 2>&1 || true
                elif docker info >/dev/null 2>&1 && command -v docker-compose >/dev/null 2>&1; then docker-compose up -d librarium >/dev/null 2>&1 || true
                elif sudo -n docker info >/dev/null 2>&1 && sudo -n docker compose version >/dev/null 2>&1; then sudo docker compose up -d librarium >/dev/null 2>&1 || true
                elif command -v docker-compose >/dev/null 2>&1 && sudo -n docker info >/dev/null 2>&1 && sudo -n docker-compose version >/dev/null 2>&1; then sudo docker-compose up -d librarium >/dev/null 2>&1 || true
                fi
            fi
        fi
...
def inspect_remote_data(target: DeploymentTarget, *, label: str | None = None) -> RemoteDataSnapshot:
    if label:
        step(f"Inspecting remote data ({label})")
    inspect_script = textwrap.dedent(f"""
        set -euo pipefail
        db={shell_quote(target.shared_dir + "/librarium.db")}
        legacy_db={shell_quote(target.shared_dir + "/codex.db")}
        vault_dir={shell_quote(target.shared_dir + "/vaults")}
        legacy_app_shared="/opt/obsidian-host/shared"
        legacy_codex_shared="/opt/codex/shared"
        legacy_app_db_present=0
        legacy_app_vault_files=0
        legacy_codex_db_present=0
        legacy_codex_vault_files=0
        legacy_codex_backups=0
        ...
        [ -f "$db" ] && db_present=1
        [ -f "$legacy_db" ] && legacy_db_present=1
        if [ -d "$vault_dir" ]; then
            vault_files=$(find "$vault_dir" -type f | wc -l | tr -d ' ')
        fi
        if [ -f "$legacy_app_shared/obsidian-host.db" ] || [ -f "$legacy_app_shared/codex.db" ]; then
            legacy_app_db_present=1
        fi
        if [ -d "$legacy_app_shared/vaults" ]; then
            legacy_app_vault_files=$(find "$legacy_app_shared/vaults" -type f | wc -l | tr -d ' ')
        fi
        if [ -f "$legacy_codex_shared/codex.db" ]; then
            legacy_codex_db_present=1
        fi
        if [ -d "$legacy_codex_shared/vaults" ]; then
            legacy_codex_vault_files=$(find "$legacy_codex_shared/vaults" -type f | wc -l | tr -d ' ')
        fi
        if [ -d "$legacy_codex_shared/backups" ]; then
            legacy_codex_backups=$(find "$legacy_codex_shared/backups" -maxdepth 1 -type f -name '*.tar.gz' | wc -l | tr -d ' ')
        fi
        if [ "$db_present" -eq 1 ] && command -v sqlite3 >/dev/null 2>&1; then
            users=$(sqlite3 "$db" "SELECT COUNT(*) FROM users;" 2>/dev/null || echo null)
            vaults=$(sqlite3 "$db" "SELECT COUNT(*) FROM vaults;" 2>/dev/null || echo null)
        elif [ "$db_present" -eq 1 ] && command -v python3 >/dev/null 2>&1; then
            counts=$(DB_PATH="$db" python3 - <<'PY'
import json
import os
import sqlite3

out = {{"users": None, "vaults": None}}
try:
    con = sqlite3.connect(os.environ["DB_PATH"])
    cur = con.cursor()
    for table, key in (("users", "users"), ("vaults", "vaults")):
        try:
...
PY
            ...
            users=$(printf '%s' "$counts" | python3 -c 'import json,sys; v=json.load(sys.stdin)["users"]; print("null" if v is None else v)' 2>/dev/null || echo null)
            vaults=$(printf '%s' "$counts" | python3 -c 'import json,sys; v=json.load(sys.stdin)["vaults"]; print("null" if v is None else v)' 2>/dev/null || echo null)
        fi
        printf '{{"db_present":%s,"legacy_db_present":%s,"user_count":%s,"vault_count":%s,"vault_file_count":%s,"legacy_app_db_present":%s,"legacy_app_vault_file_count":%s,"legacy_codex_db_present":%s,"legacy_codex_vault_file_count":%s,"legacy_codex_backup_count":%s}}\\n' \\
            "$db_present" "$legacy_db_present" "$users" "$vaults" "$vault_files" "$legacy_app_db_present" "$legacy_app_vault_files" "$legacy_codex_db_present" "$legacy_codex_vault_files" "$legacy_codex_backups"
    """).strip()
    r = ssh_cmd(target, inspect_script, capture=True)
    raw = r.stdout.strip().splitlines()[-1]
    data = json.loads(raw)
    snapshot = RemoteDataSnapshot(
        db_present=bool(data["db_present"]),
        legacy_db_present=bool(data["legacy_db_present"]),
        user_count=data["user_count"],
        vault_count=data["vault_count"],
        vault_file_count=int(data["vault_file_count"]),
        legacy_app_db_present=bool(data["legacy_app_db_present"]),
        legacy_app_vault_file_count=int(data["legacy_app_vault_file_count"]),
        legacy_codex_db_present=bool(data["legacy_codex_db_present"]),
        legacy_codex_vault_file_count=int(data["legacy_codex_vault_file_count"]),
        legacy_codex_backup_count=int(data["legacy_codex_backup_count"]),
    )
    info(
        "Remote data: "
        f"db={'yes' if snapshot.db_present else 'no'}, "
        f"users={snapshot.user_count if snapshot.user_count is not None else 'unknown'}, "
        f"vaults={snapshot.vault_count if snapshot.vault_count is not None else 'unknown'}, "
        f"vault files={snapshot.vault_file_count}"
        + (", legacy codex.db present" if snapshot.legacy_db_present else "")
        + (", legacy /opt/obsidian-host data present" if snapshot.legacy_app_db_present or snapshot.legacy_app_vault_file_count else "")
        + (", legacy /opt/codex data present" if snapshot.legacy_codex_db_present or snapshot.legacy_codex_vault_file_count else "")
    )
    return snapshot


def _known_count_dropped(before: int | None, after: int | None) -> bool:
    return before is not None and after is not None and before > 0 and after < before


def _data_restore_reason(before: RemoteDataSnapshot, after: RemoteDataSnapshot) -> str | None:
    if before.db_present and not after.db_present:
        return "database file disappeared"
    if _known_count_dropped(before.user_count, after.user_count):
        return "user count dropped"
    if _known_count_dropped(before.vault_count, after.vault_count):
        return "vault count dropped"
    if before.vault_file_count > 0 and after.vault_file_count < before.vault_file_count:
        return "vault file count dropped"
    return None


def restore_remote_backup(target: DeploymentTarget, backup_path: str, *, reason: str) -> None:
    systemd_services = " ".join(shell_quote(s) for s in _SERVICE_CANDIDATES)
    step("Restoring remote backup")
    warn(f"Post-deploy data check failed: {reason}. Restoring {backup_path}")
    restore_script = textwrap.dedent(f"""
        ...
        tar -xzf {shell_quote(backup_path)} -C {shell_quote(target.app_dir)} shared
    ...
    compose = textwrap.dedent(f"""
        services:
          librarium:
            ...
            image: librarium:{target.name}
            container_name: librarium-{target.name}
            ports:
            ...
            environment:
              ...
              LIBRARIUM_INDEX_DIR: "/data/indices"
              RUST_LOG: "info,librarium=info,actix_web=info"
...
def deploy_to_target(target: DeploymentTarget, artifacts: BuildArtifacts, *, skip_backup: bool = False) -> DeployResult:
    ensure_tool("ssh")
    ensure_tool("scp")
    ensure_remote_layout(target)
    backup_path: str | None = None
    before_data: RemoteDataSnapshot | None = None
    data_restored = False
    if skip_backup:
        warn("Skipping pre-deploy backup because --skip-backup was set")
    else:
        before_data = inspect_remote_data(target, label="before deploy")
        backup_path = create_remote_backup(target, label="pre-deploy", stop_services=True, restart_after=False)
    ...
    if backup_path and before_data:
        after_data = inspect_remote_data(target, label="after deploy")
        restore_reason = _data_restore_reason(before_data, after_data)
        if restore_reason:
            restore_remote_backup(target, backup_path, reason=restore_reason)
            data_restored = True
            healthy = healthcheck(target)
            inspect_remote_data(target, label="after restore")
        else:
            ok("Post-deploy data check passed")
    ...
    if gen_user or gen_pass:
        ...
        if bootstrap_status == "skipped":
            console.print(Panel(
                ...
                "  • SSH in and delete [cyan]shared/librarium.db[/cyan] to reset, then restart the service\n"
                f"  • Run [cyan]librarium.py configure --target <name> --reset-admin[/cyan]",
...
def deploy_to_docker_target(target: DeploymentTarget, *, skip_backup: bool = False) -> DeployResult:
    ...
    _remote_compose_command(target, "up -d --build librarium")
...
def _print_admin_credentials(username: str, password: str, *, title: str = "Admin credentials") -> None:
    console.print(Panel(
    ...
    *,
    ...
    for line in lines:
        ...
        if section == "server" and stripped.startswith("host ="):
            ...
            rendered.append('path = "./librarium.db"')
...
def _patch_bootstrap_creds(config_text: str) -> tuple[str, bool, str | None, str | None]:
    ...
    if existing_user and existing_pass:
        return config_text, False, None, None
    default_user, gen_pass = _generate_credentials()
    final_user = existing_user or default_user
    ...
    if config_text.endswith("\n"):
        updated += "\n"
    return updated, True, final_user if not existing_user or not existing_pass else None, gen_pass if not existing_pass else None
...
def _ensure_docker() -> None:
    ...
    try:
        ...
        raise LibrariumError(f"Docker daemon not reachable. Start Docker and retry. ({e})") from e
    ...
    if dest_cfg.exists():
        ...
        for line in lines:
            ...
            if section == "server" and stripped.startswith("host ="):
                ...
                rendered.append(f'path = "{config_dir / "librarium.db"}"')
            elif section == "auth" and stripped.startswith("enabled ="):
                rendered.append("enabled = true")
...
def cli() -> None:
    """Librarium — build, deploy, configure, and test the Librarium server and desktop."""
...
@click.option("--skip-support", is_flag=True, help="Skip building librarium-client / librarium-types")
@click.pass_context
def build(
    ctx: click.Context,
    component: str,
...
@click.option("--config-dir", type=click.Path(path_type=Path), default=None, metavar="DIR",
              help="Directory to install config.toml into (default: ~/.config/librarium)")
...
@click.option("--admin-password", default=None, metavar="PASS", envvar="LIBRARIUM_ADMIN_PASSWORD",
              help="Bootstrap admin password written to config (env: LIBRARIUM_ADMIN_PASSWORD)")
...
def local_install_cmd(
    ...
    """Build and install Librarium locally.

    Copies the server binary to INSTALL_DIR (default: ~/.local/bin) and writes
    a starter config.toml to CONFIG_DIR (default: ~/.config/librarium).
    ...
    \b
      ...
      python scripts/librarium.py local-install --admin-password mysecret

    \b
      # Install from an already-built dist/ without rebuilding:
      python scripts/librarium.py local-install --no-build

    \b
      # Install to /usr/local/bin (may require sudo for the copy):
      python scripts/librarium.py local-install --install-dir /usr/local/bin
    """
...
@click.option("--skip-backup", is_flag=True, help="Skip the pre-deploy remote backup and post-deploy restore guard")
...
def deploy(
    ...
    """Deploy Librarium server to a remote target.
    ...
    try:
        ...
        if target.deployment == "docker":
            ...
            result = deploy_to_target(target, artifacts, skip_backup=skip_backup)
        else:
        ...
        rows = [
            ...
            ("Data restored from backup", result.data_restored),
        ]
...
def migrate(
    ...
    """Remove legacy obsidian-host service and redeploy as librarium.

    Stops and removes the old 'obsidian-host' systemd unit, then runs a full
    deploy so the server runs under the 'librarium' service name.
    """
    try:
        targets = load_targets(targets_file)
        target = pick_target(target_name, targets)
        ...
        # ── 4. Deploy librarium ───────────────────────────────────────────────
        step("Deploying librarium service")
        artifacts = load_existing_artifacts(target)
...
def _reset_remote_admin(target: DeploymentTarget) -> None:
    ...
    db_path = f"{target.shared_dir}/librarium.db"
    db_backup = f"{target.shared_dir}/librarium.db.bak"
    ...
    if bootstrap_status == "created":
        ...
        info("Bootstrap status unknown — check logs with: librarium.py logs --target <name>")

    _print_admin_credentials(username, password, title="New admin credentials")


def _start_remote_service(target: DeploymentTarget) -> None:
    if target.deployment == "docker":
        _remote_compose_command(target, "up -d librarium")
        ok("Docker Compose deployment running")
        return
    svc = detect_service_name(target)
    ssh_cmd(target, f"sudo systemctl start {shell_quote(svc)}")
...
def _rename_remote_user(target: DeploymentTarget, old_username: str, new_username: str) -> None:
    """Rename an existing user in the remote SQLite DB after taking a backup."""
    old_username = old_username.strip()
    new_username = new_username.strip()
    username_pat = re.compile(r"^[A-Za-z0-9_.@-]{1,64}$")
    if not old_username or not new_username:
        raise LibrariumError("Both old and new usernames are required")
    if old_username == new_username:
        raise LibrariumError("Old and new usernames are identical")
    if not username_pat.match(new_username):
        raise LibrariumError("New username must be 1-64 characters using letters, numbers, '.', '_', '@', or '-'")

    db_path = f"{target.shared_dir}/librarium.db"
    if not _remote_path_exists(target, db_path):
        raise LibrariumError(f"Remote database not found: {db_path}")

    warn("This will back up persistent data, stop Librarium, rename one user, and restart Librarium.")
    click.confirm("Continue?", abort=True)

    backup_path = create_remote_backup(target, label="pre-rename-user", stop_services=True, restart_after=False)

    try:
        step("Renaming remote user")
        rename_script = textwrap.dedent(f"""
            set -euo pipefail
            export OLD_USERNAME={shell_quote(old_username)}
            export NEW_USERNAME={shell_quote(new_username)}
            export DB_PATH={shell_quote(db_path)}
            python3 - <<'PY'
import json
import os
import sqlite3
...
old_username = os.environ["OLD_USERNAME"]
...
try:
    ...
    if old_row is None:
        print(json.dumps({{"status": "missing_old"}}))
        sys.exit(2)

    cur.execute("SELECT id FROM users WHERE username = ?", (new_username,))
    if cur.fetchone() is not None:
        print(json.dumps({{"status": "new_exists"}}))
        sys.exit(3)

    cur.execute("UPDATE users SET username = ? WHERE username = ?", (new_username, old_username))
    con.commit()
    print(json.dumps({{"status": "renamed", "user_id": old_row[0], "old_username": old_username, "new_username": new_username}}))
finally:
    con.close()
PY
        """).strip()
        r = ssh_cmd(target, rename_script, capture=True, check=False)
        output = (r.stdout or "").strip().splitlines()
        detail = output[-1] if output else ""
        if r.returncode != 0:
            try:
                status = json.loads(detail).get("status")
            except json.JSONDecodeError:
                status = None
            if status == "missing_old":
                raise LibrariumError(f"User {old_username!r} was not found")
            if status == "new_exists":
                raise LibrariumError(f"User {new_username!r} already exists")
            raise LibrariumError((r.stderr or detail or f"Rename failed with exit code {r.returncode}").strip())

        result = json.loads(detail)
        ok(f"Renamed user {result['old_username']!r} to {result['new_username']!r}")
        info(f"Backup created before rename: {backup_path}")
    finally:
        step("Starting service")
        _start_remote_service(target)
...
@click.option("--rename-user", nargs=2, metavar="OLD NEW", help="Rename an existing remote user in the database")
@click.option("--targets-file", type=click.Path(path_type=Path), default=TARGETS_FILE, show_default=True)
def configure(
    target_name: str | None,
    show: bool,
    ...
    reset_admin: bool,
    rename_user: tuple[str, str] | None,
    targets_file: Path,
) -> None:
    """Manage local or remote Librarium configuration."""
    try:
        if target_name:
            targets = load_targets(targets_file)
            target = pick_target(target_name, targets)
            ...
            if init:
                ...
                _print_admin_credentials(username, password)
                return
            ...
            if bootstrap_admin:
                ...
                if not changed:
                    ...
                    if gen_user and gen_pass:
                        _print_admin_credentials(gen_user, gen_pass)
                    elif gen_user:
                        info(f"Bootstrap admin username set to {gen_user!r}")
                return
            ...
            if rename_user:
                old_username, new_username = rename_user
                _rename_remote_user(target, old_username, new_username)
                return

            console.print("Specify --show, --init, --bootstrap-admin, --reset-admin, or --rename-user OLD NEW")
        else:
            if reset_admin or bootstrap_admin or init or rename_user:
...
def test(
    ...
    if suite in ("desktop", "all"):
        def _desktop() -> None:
            run_cmd(
                ["cargo", "test", "-p", "librarium-desktop", "--tests", *filter_args],
                cwd=REPO_ROOT,
            )
...
def data_status(
    ...
    """Inspect remote database and uploaded vault data counts."""
    try:
        all_targets = load_targets(targets_file)
        target = pick_target(target_name, all_targets)
        snapshot = inspect_remote_data(target)
        table = Table(title=f"Remote data: {target.name}", box=box.SIMPLE_HEAVY)
        table.add_column("Item")
        table.add_column("Value")
        table.add_row("Database present", "yes" if snapshot.db_present else "no")
        table.add_row("Users", str(snapshot.user_count) if snapshot.user_count is not None else "unknown (sqlite3 unavailable or table missing)")
        table.add_row("Vaults", str(snapshot.vault_count) if snapshot.vault_count is not None else "unknown (sqlite3 unavailable or table missing)")
        table.add_row("Vault files", str(snapshot.vault_file_count))
        table.add_row("Legacy codex.db present", "yes" if snapshot.legacy_db_present else "no")
        table.add_row("Legacy /opt/obsidian-host DB", "yes" if snapshot.legacy_app_db_present else "no")
        table.add_row("Legacy /opt/obsidian-host vault files", str(snapshot.legacy_app_vault_file_count))
        table.add_row("Legacy /opt/codex DB", "yes" if snapshot.legacy_codex_db_present else "no")
        table.add_row("Legacy /opt/codex vault files", str(snapshot.legacy_codex_vault_file_count))
        table.add_row("Legacy /opt/codex backups", str(snapshot.legacy_codex_backup_count))
        console.print(table)
        if snapshot.legacy_db_present and not snapshot.db_present:
            warn("Legacy codex.db exists but librarium.db does not; data may be orphaned by the rename.")
        if snapshot.legacy_app_db_present or snapshot.legacy_app_vault_file_count:
            warn("Legacy /opt/obsidian-host data exists; review it before deleting that directory.")
        if snapshot.legacy_codex_db_present or snapshot.legacy_codex_vault_file_count:
            warn("Legacy /opt/codex data exists; this is likely the pre-rename persisted state.")
    except LibrariumError as e:
        fail(str(e))
        raise SystemExit(1)
...
@cli.command()
@click.argument("target_name", metavar="TARGET", required=False)
@click.option("--token", envvar="LIBRARIUM_TOKEN", default=None, metavar="JWT", help="Bearer token for authenticated requests (or set LIBRARIUM_TOKEN)")
@click.option("--targets-file", type=click.Path(path_type=Path), default=TARGETS_FILE, show_default=True)
def status(
    target_name: str | None,
    token: str | None,
    ...
    try:
        ...
        if target.deployment == "docker":
            ...
            try:
                r3 = _remote_compose_command(target, "ps librarium", capture=True)
        ...
        def http_get(path: str) -> tuple[int, dict]:
            ...
            try:
                ...
                raise LibrariumError(f"Cannot reach {base_url}: {e}") from e
        ...
        try:
            ...
            if code == 200:
                ...
                health_note = "(auth required — pass --token or set LIBRARIUM_TOKEN)"
        ...
        console.print(Panel(table, title=f"[bold]librarium status — {target.name}[/bold]", border_style=service_color))
...
def logs(
    ...
    try:
        ...
        if target.deployment == "docker":
            ...
            _remote_compose_command(target, f"logs {follow_flag} --tail {lines} librarium")
            return
